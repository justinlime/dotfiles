...
(use-package evil-collection
  :after evil
  :config
    (setq evil-collection-mode-list '(magit doc-view ement term minibuffer help dashboard dired ibuffer tetris gptel eat))
...
#+begin_src emacs-lisp
(use-package eat
  :general (:keymaps 'eat-mode-map
            :states 'insert
              ; Send tab inputs properly to eat
              "TAB" (lambda () (interactive) (eat--send-input nil "\t"))
              "<tab>" (lambda () (interactive) (eat--send-input nil "\t")))
...
(use-package gptel
  ...
  ;; visual-line-mode
    ...
    :config
      ...
      ;; (gptel-mcp-connect)
      (setq gptel-backend
        (gptel-make-openai "StinkGPT"
          :protocol "http"
          :host "stink.local:8282"
          :stream t
        ...
        gptel-stream t
        gptel-model "StinkGPT"
        gptel-default-mode #'org-mode
        gptel--system-message 
        "You are a LLM that lives in Emacs. Respond conciesly. 
         All responses must be formatted in org mode, such as: 
         - headers
         - bold text
         - italic text
         - quotes
...
#+begin_src emacs-lisp
(use-package minuet
  :vc (minuet :url "https://github.com/milanglacier/minuet-ai.el"
              :rev "10638b986ee83b15856f89fafac4ea9a689bf67f")
  :bind
    ...
    :config
      ...
      (setq minuet-context-window 11000)
      (setq minuet-request-timeout 10)
      (setq minuet-auto-suggestion-mode t)


      ;; (plist-put minuet-openai-fim-compatible-options :end-point "http://stink.local:1313/v1/completions")
      (plist-put minuet-openai-fim-compatible-options :end-point "http://stink.local:8282/v1/completions")
      (plist-put minuet-openai-fim-compatible-options :name "StinkGPT")
      (plist-put minuet-openai-fim-compatible-options :api-key "TERM")
      (plist-put minuet-openai-fim-compatible-options :model "StinkGPT")
      (minuet-set-optional-options minuet-openai-fim-compatible-options :stream t)
      (minuet-set-optional-options minuet-openai-fim-compatible-options :temperature 0.2)
      (minuet-set-optional-options minuet-openai-fim-compatible-options :top_p 0.8)
      (minuet-set-optional-options minuet-openai-fim-compatible-options :min_p 0)
      (minuet-set-optional-options minuet-openai-fim-compatible-options :top_k 20)
...
(use-package pyvenv
  :hook 
    (python-ts-mode . (lambda ()
      (let ((venv-dir (concat (vc-root-dir) ".venv")))
