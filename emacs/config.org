...
(defun toggle-term ()
  "Toggle a terminal window with $SHELL"
  (interactive)
  (let (
    (w (get-buffer-window "*terminal*"))
    (h (window-total-height)))
      (cond (
        (eq nil w)(select-window (split-window-below (round (* h 0.80))))(term (getenv "SHELL"))(evil-insert 1))
        (t (delete-window w))))
)
...
(add-hook 'prog-mode-hook (lambda()
  (unless (file-remote-p (buffer-file-name))
    (cond
      ...
      ((eq major-mode 'ts-mode)(eglot-ensure))
    )
  )
))
#+end_src
...
(use-package corfu
  ...
  :bind nil
  :config
  ;; Prevents evil from fucking with corfu-maps
  (evil-make-overriding-map corfu-map)  ;; Org
