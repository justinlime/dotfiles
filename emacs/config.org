...
(defvar active-toggle-term "*term*" "The most recently used toggle term")
(defvar current-toggles '() "List of the current toggle terms")
  ...
  (let* (
    ...
    (height (window-total-height)))
      (if window (delete-window window)
          (select-window (split-window-below (round (* height 0.80))))
...
;;           (if (member name current-toggles) nil (add-to-list 'current-toggles name))
...
(use-package general
  ...
  (leader
    ...
    "g c" '(toggle-active-term :wk "Get Current Terminal")
    "g a" '(toggle-term :wk "Get Terminal")
