...
(use-package evil
  ...
  :config
    ...
    (unless (display-graphic-p)
      ...
      (run-with-idle-timer nil 1 #'correct-cursor)
...
(use-package evil-collection
  :after evil
  :config
    (setq evil-collection-mode-list '(magit term minibuffer help dashboard dired ibuffer tetris))
...
(advice-add #'wdired-exit :after #'(lambda (&rest r) (dired-revert)))
#+end_src
*** Diredfl
#+begin_src emacs-lisp
...
(use-package corfu
  ...
  :config
    ...
    ;; `corfu-insert-separator' is needed for orderless matching
    ;; 1.) insert space if nothing is selected, to continue orderless matching 
    ;; 2.) if option is selected, insert it, along with a space
    ;; 3.) quit corfu if there are no matches when entering the seperator
...
*** Rainbow Mode
#+begin_src emacs-lisp
(use-package rainbow-mode :config
