...
(use-package toggle-term
  ...
  :config
    ...
    (setq toggle-term-init-toggle '("emacs" . "term"))
...
(defun jl/ssh (host formatter)
  ...
  (let ((format-host (format formatter host host)))
    ...
    (toggle-term-find host "term")
    (mapcar #'(lambda (char) (execute-kbd-macro (char-to-string char))) (format "ssh %s" host))
...
(with-eval-after-load 'eglot
  (mapcar #'(lambda (server-remap)
              (add-to-list 'eglot-server-programs server-remap))
    '(((java-ts-mode java-mode) . ("java-language-server"))
      ((nix-ts-mode nix-mode) . ("nixd")))))
;; Enable modes for eglot
(add-hook 'find-file-hook #'(lambda()
  (unless (file-remote-p (buffer-file-name)) 
    (mapcar #'(lambda (eglot-enabled-mode)
                (if (eq major-mode eglot-enabled-mode)
                  (eglot-ensure)))
