...
(advice-add #'dired-do-delete :after #'jl/clear-marks)
(advice-add #'dired-do-rename :after #'jl/clear-marks)
(advice-add #'dired-do-flagged-delete :after #'jl/clear-marks)
(general-define-key
  :states 'normal
...
(use-package dired-ranger
  ...
  :config
    ...
    (advice-add #'dired-ranger-move :after #'jl/clear-marks)
    (advice-add #'dired-ranger-paste :after #'jl/clear-marks)
...
(use-package dired-subtree
  ...
  :config 
    ...
    (add-hook 'dired-subtree-after-insert-hook #'(lambda ()
      (dired-omit-mode 1)))
    (mapcar #'(lambda (depth) 
                (set-face-attribute (read (format "dired-subtree-depth-%s-face" depth)) nil :background nil)) '(1 2 3 4 5 6)))
#+end_src
*** Dired Sidebar
#+begin_src emacs-lisp
...
(with-eval-after-load 'eglot
  (mapcar #'(lambda (server-prog)
              (add-to-list 'eglot-server-programs server-prog))
...
(add-hook 'find-file-hook #'(lambda()
    (unless (file-remote-p (buffer-file-name)) 
      (dolist (lang '(go-ts-mode python-ts-mode js-ts-mode
                      ...
                      nix-ts-mode csharp-ts-mode java-ts-mode c-ts-mode
                      bash-ts-mode))
...
(mapcar #'(lambda (lang) (unless (treesit-language-available-p (car lang))
                      (treesit-install-language-grammar (car lang)))) treesit-language-source-alist)

;; maps the ts modes to normal modes
(mapcar #'(lambda (mode-remap) (add-to-list 'major-mode-remap-alist mode-remap))
  '((c-mode . c-ts-mode)
    (c++-mode . c++-ts-mode)
...
(mapcar #'(lambda (auto-mode) (add-to-list 'auto-mode-alist auto-mode))
  '(("\\.go\\'" . go-ts-mode)
    ("\\.rs\\'" . rust-ts-mode)
    ("\\.toml\\'" . toml-ts-mode)
    ("\\.yml\\'" . yaml-ts-mode)
