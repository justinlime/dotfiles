...
(defun jl/random-icon ()
  ...
  (interactive)
  (let* ((ops (directory-files (expand-file-name "icons/" user-emacs-directory)))
        (ops (delete "." ops))
        (ops (delete ".." ops))) (nth (random (length ops)) ops)))
#+End_src
...
(use-package dashboard
  ...
  (setq default-directory "~/"
        initial-buffer-choice (lambda() (get-buffer-create "*dashboard*")) ;; Start dashboard for emacs clients
        dashboard-banner-logo-title (jl/random-quote)
        dashboard-footer-messages `(,(jl/random-quote)))
  (when (display-graphic-p)
    (setq dashboard-startup-banner (format "~/.config/emacs/icons/%s" (jl/random-icon)))))
