...
(defvar jl/active-toggle-term "*toggle-terminal*" "The most recently used toggle term")
(defvar jl/current-toggles '("*toggle-terminal*") "List of the current toggle terms")
          ...
          (window (get-buffer-window wrapped-name)))
            (if window (delete-window window)
                ...
                (if (eq command nil)
                  ...
                  (make-term unwrapped-name (getenv "SHELL") nil "-c" (format "ssh %s" name))) 
